...
Private Sub cmdMedicalCert_Click()
    ...
    patientID = InputBox("Enter Patient ID to generate certificate:")
    If patientID = "" Then Exit Sub

    Dim rsCert As New ADODB.Recordset
    rsCert.Open "SELECT Name, Diagnosis FROM patient_master WHERE ID = " & patientID, _
                cn, adOpenStatic, adLockReadOnly
...
Private Sub cmdPrint_Click()
    ...
    Do While Not rsReport.EOF
        ...
        For i = 0 To rsReport.Fields.Count - 1
            rowText = rowText & rsReport.Fields(i).Name & ": " & _
                      rsReport.Fields(i).Value & "   "
        Next i
...
Private Sub cmdExportPDF_Click()
    ...
    Do While Not rsReport.EOF
        ...
        For i = 0 To rsReport.Fields.Count - 1
            If IsNull(rsReport.Fields(i).Value) Then
                rowText = rowText & ""
            Else
