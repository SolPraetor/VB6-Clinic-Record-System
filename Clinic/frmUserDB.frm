...
Public OpenLogOnLoad As Boolean

'Necessary Codes
Public Sub LoadMedicineCombo(cbo As ComboBox)
...
Private Sub cmdSelect_Click()
    ...
    Set rs = New ADODB.Recordset
    rs.Open "SELECT * FROM patient_master WHERE ID = " & SelectedPatientID, cn, adOpenDynamic, adLockOptimistic
...
Private Sub cmdRConfirm_Click()
    ...
    If MsgBox("Are you sure you want to delete this record?", vbYesNo + vbQuestion) = vbYes Then
        ...
        treatmentVal = IIf(IsNull(rs!Treatment), "NULL", "'" & Replace(rs!Treatment, "'", "''") & "'")
        diagnosisVal = IIf(IsNull(rs!Diagnosis), "NULL", "'" & Replace(rs!Diagnosis, "'", "''") & "'")
        medicineVal = IIf(IsNull(rs!Medicine), "NULL", "'" & Replace(rs!Medicine, "'", "''") & "'")
        ...
        sqlArchive = "INSERT INTO archive_master (ID, Name, Address, Age, DOB, Sex, Allergy, Condition, Contact, Complain, Treatment, Diagnosis, Medicine) VALUES (" & _
                     ...
                     Replace(rs!Complain, "'", "''") & "', " & treatmentVal & ", " & diagnosisVal & ", " & medicineVal & ")"
...
Private Sub cmdLoadArchive_Click()
    ...
    Dim i As Integer
    For i = 0 To 12
        DGArchive.Columns(i).Width = 1500
    Next i
End Sub
...
Private Sub cmdEdit_Click()
    ...
    frmEditData.OpenPrescriptionOnLoad = False
